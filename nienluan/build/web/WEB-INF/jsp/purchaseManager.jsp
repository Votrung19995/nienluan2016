...
<html>
    <head>
        ...
        <title>Trang quản lý hóa đơn mua hàng</title>
        ...
        <body onload="startTime()">
            ...
            <div class="container" style="width:100%">
                ...
                <div class="row">
                    ...
                    <p style="margin-left:10px; " class="p"><a href="goQuanly.html?tk=${kh.user}">quản lý</a></p>
                    <p style="margin-left:10px;color:blue" class="p">></p>
                    <p style="margin-left:10px; " class="p"><a href="gopurchaseManager.html?tk=${kh.user}">hóa đơn mua hàng</a></p>
                    </div>
...
//                                                       });
            ...
            <hr>
                <div class="col-sm-10 col-sm-offset-1">
                    ...
                    <center>
                        <h3 id="an" class="text-muted" style="display:none">Danh sách các hóa đơn mua hàng của khách hàng</h3>
                    </center>
                    <br><br>
                    ...
                    <table class="table table-hover table-bordered table-responsive table-striped">
                        <thead>
                            <tr style="background-color:#CEF6CE">
                                <th style="text-align:center" class="text-muted">STT</th>
                                <th style="text-align:center" class="text-muted">Mã hóa đơn</th>
                                <th style="text-align:center" class="text-muted">Tên khách hàng</th>
                                <th style="text-align:center" class="text-muted">Ngày đặt hàng</th>
                                <th style="text-align:center" class="text-muted">Tổng thanh toán</th>
                                <th style="text-align:center" class="text-muted">Đã nhận hàng</th>
                                <th style="text-align:center" class="text-muted">Đuyệt hóa đơn</th>
                                <th style="text-align:center" class="text-muted">Chi tiết</th>
                            </tr>
                        </thead>
                        <tbody>
                            <c:forEach items="${lpur}" var="lis" varStatus="i">
                                <tr>
                                    <td style="text-align:center"class="text-muted">${i.count}</td>
                                    <td style="text-align:center"class="text-muted">${lis.purchaseId}</td>
                                    <td style="text-align:center"class="text-muted">${lis.purchaseName}</td>
                                    <td style="text-align:center"class="text-muted">${lis.purchaseDate}</td>
                                    <td style="text-align:center"class="text-muted">${lis.total} VNĐ</td>
                                    
                                    <c:choose>
                                        <c:when test="${lis.give==0}">
                                            <td style="text-align:center"class="text-muted"><i class="fa fa-hand-paper-o" style="color:orangered"></i> Chưa nhận hàng</td>
                                        </c:when>
                                        <c:otherwise>
                                            <td style="text-align:center"class="text-muted"><span class="glyphicon glyphicon-download-alt" style="color:blue"></span> Đã nhận hàng</td>
                                        </c:otherwise>
                                    </c:choose>
                                            
                                    <c:choose>
                                        <c:when test="${lis.status==0}">
                                            <td style="text-align:center"class="text-muted">
                                                <a href="duyetPur.html?tk=${kh.user}&id=${lis.purchaseId}&name=${kh.name}" title="duyệt hóa đơn này" class="btn btn-block btn-primary"> Duyệt ngay</a>
                                            </td>
                                        </c:when>
                                        <c:otherwise>
                                            <td style="text-align:center"class="text-muted"><span style="color:green" class="glyphicon glyphicon-check"></span> Đã duyệt</td>
                                        </c:otherwise>
                                    </c:choose>
                                            <td style="text-align:center"><a href="golistPur.html?id=${lis.purchaseId}&tk=${kh.user}"><span class="glyphicon glyphicon-list-alt"></span></a></td>      
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
            ...
            <div class="row">
                ...
                <a href="goQuanly.html?tk=${kh.user}" class="btn-sm pull-left">
